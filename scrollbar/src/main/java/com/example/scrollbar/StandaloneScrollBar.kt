...
class StandaloneScrollBar : FrameLayout {
    ...
    private var _scrollBarTotalIndex = 0f
    ...
    var scrollBarSize: Float
        get() = _scrollBarTotalIndex
        set(value) {
            if (value != _scrollBarTotalIndex) {
                _scrollBarTotalIndex = value
                requestLayout()
            }
        }
    ...
    private fun init(attributeSet: AttributeSet? = null, defStyleAttr: Int = 0) {
        context.theme.obtainStyledAttributes(
            attributeSet,
            R.styleable.StandaloneScrollBar, defStyleAttr, 0
        ).use {
            ...
            _scrollBarTotalIndex =
                it.getFloat(
                    R.styleable.StandaloneScrollBar_scrollBarTotalItem,
                    Float.NaN
                )
            autoThumbLength =
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (thumbLength >= 0 || thumbLengthByTrackRatio >= 0 || autoThumbLength || minThumbLength > 0) {
            var (thumbWidthMeasureSpec, thumbHeightMeasureSpec) = orientationHelper.getThumbMeasureSpec()
            thumbWidthMeasureSpec = R.dimen.min_thumb_width
